...
import { CredentialAcceptRequest } from '../../types/api';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { subjectSeed, issuerAddress, credentialType }: CredentialAcceptRequest = req.body;
    ...
    res.status(200).json(createSuccessResponse(result, 'Credential accepted successfully'));
  } catch (error) {
